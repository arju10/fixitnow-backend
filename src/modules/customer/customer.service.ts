...
export const updateCustomerProfile = async (userId: string, input: UpdateCustomerProfileInput) => {
  // Update User fields
  const userData: any = {};
  ...
  if (input.address !== undefined) profileData.address = input.address || null;
  if (input.city !== undefined) profileData.city = input.city || null;
  if (input.postalCode !== undefined) profileData.postalCode = input.postalCode || null;
